...
NUMBER_WORDS = {
    ...
    "fifteen": 15, "twenty": 20, "thirty": 30, "seventy-two": 72,
}
...
def classify_impact(context: FindingContext) -> tuple[str, str]:
    ...
    meaningful_reg_terms = {
        ...
        if t not in {"shall", "must", "required", "within", "days", "policy", "data"}
    }
    meaningful_int_terms = set(re.findall(r"[a-z]{4,}", int_lower))
    shared_terms = meaningful_reg_terms & meaningful_int_terms
    shared_topics = {
        "incident", "breach", "reported", "reporting", "notification", "notify",
        "encryption", "security", "vendor", "processor", "subprocessor",
        "retention", "retained", "deletion", "erasure", "erase", "foreclosure",
        "prepayment", "kyc", "grievance", "interest", "borrower",
    } & shared_terms
    ...
    if (
        ...
        and not reg_negates
        and any(term in reg_lower for term in ["mechanism", "deletion", "erasure", "erase", "waive"])
    ):
        return (
            "contradicts",
...
def draft_redline(context: FindingContext, impact_type: str) -> str:
    ...
    if "mechanism" in int_lower and ("does not currently provide" in int_lower or "no mechanism" in int_lower):
        ...
        if "five (5) years" in reg_lower or "5 years" in reg_lower:
            required_match = re.search(r"\b(\d+)\s*\(?\d*\)?\s*years?\b", reg_lower)
            required_period = required_match.group(1) if required_match else "the required"
            proposed_text = re.sub(
...
def generate_business_action(context: FindingContext, impact_type: str) -> str:
    ...
    if any(term in reg_lower for term in ["erase", "erasure", "deletion"]) and any(
        ...
        any(term in reg_lower for term in ["mechanism", "deletion", "erasure", "erase"])
    ...
    elif ("retained" in int_lower or "retention" in int_lower) and "year" in reg_lower:
        internal_periods = _extract_numbers(int_lower)
        regulation_periods = _extract_numbers(reg_lower)
        current_period = str(internal_periods[0]) if internal_periods else "the current"
        required_period = str(regulation_periods[-1]) if regulation_periods else "the required"
        action_summary = f"Update the policy retention period from {current_period} years to {required_period} years."
